...
let buildMigratorArgs parameters assembly =
    ...
    let task = match parameters.Task with
                | MigrationTask.Migrate -> "migrate"
                | MigrationTask.MigrateDown -> "migrate:down"
                | MigrationTask.Rollback -> "rollback"
                | MigrationTask.RollbackToVersion -> "rollback:toversion"
                | MigrationTask.RollbackAll -> "rollback:all"
                | MigrationTask.ListMigrations -> "listmigrations"
                | _ -> "migrate"
    ...
    new StringBuilder()
        ...
        |> appendIfTrue (parameters.Task = MigrationTask.Rollback) (sprintf "--steps=%d" parameters.Steps)
        |> append (sprintf "--task=%s" task)
        ...
        |> appendIfTrue (parameters.Profile <> null) (sprintf "--profile=%s" parameters.Profile)
